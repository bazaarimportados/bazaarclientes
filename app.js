...
// ---------------------------------------------------------------
function show(id){
  ['loading-screen','login-screen','denied-screen','app-root'].forEach(x=>{
...
onAuthStateChanged(auth, async (user)=>{
  ...
  if(!allowed){
    document.getElementById('denied-email').textContent = `Você entrou como ${user.email}.`;
    show('denied-screen');
    return;
  }
...
async function renderObsTab(c){
  ...
  document.getElementById('obs-save').addEventListener('click', async ()=>{
    ...
    list.push({ text, tag: currentObsTag, date: new Date().toLocaleString('pt-BR'), by: (auth.currentUser && auth.currentUser.displayName) || '' });
    try{
